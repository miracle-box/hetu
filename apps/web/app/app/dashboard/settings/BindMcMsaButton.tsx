...
export function BindMcMsaButton() {
  ...
  const createVerificationMutation = useMutation({
    ...
    onSuccess: (resp) => {
      respToEither(resp).ifRight(({ verification }) => {
  ...
  return (
    <Button onClick={() => createVerificationMutation.mutate()} disabled={createVerificationMutation.isPending}>
      {createVerificationMutation.isPending ? "跳转中..." : "绑定 Minecraft 账号（Mojang 正版）"}
    </Button>
  );
}
